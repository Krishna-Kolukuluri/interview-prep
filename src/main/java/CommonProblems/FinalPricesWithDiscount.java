...
the shop, if you buy the ith item, then you will receive a discount equivalent to prices[j] where j is the minimum index
...
public class FinalPricesWithDiscount {
    public static void main(String[] args) {
        ...
        int[] finalPrices = getFinalPrices(prices);
        for (int fp: finalPrices){
    ...
    private static int[] getFinalPrices(int[] prices){
        int l = prices.length;
        int[] finaPrices = new int[l];
        for(int i=0;i<l;i++){
            boolean foundDiscount = false;
            for(int j=i+1;j<l;j++){
                if(prices[j]<= prices[i]){
                    finaPrices[i] = prices[i] - prices[j];
                    foundDiscount = true;
                    break;
                }
            }
            if (!foundDiscount){
                finaPrices[i] = prices[i];
            }
        }
        return finaPrices;
    }
